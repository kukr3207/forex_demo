...
bond,usd2y, usd5y, usd10y = st.tabs(["Bonds Overall","USD2Y", 'USD5Y', "USD10Y"])

with bond:
   l = ['UST 2Y NOTE - CHICAGO BOARD OF TRADE','UST 10Y NOTE - CHICAGO BOARD OF TRADE','UST 5Y NOTE - CHICAGO BOARD OF TRADE']

   l1 = ['USD2Y','USD10Y', 'USD5Y']
...
with usd2y:
   st.header("USD2Y Smart Money tracker")
   df_data = df[df["Asset"] == 'UST 2Y NOTE - CHICAGO BOARD OF TRADE'].iloc[:,1:]
...
with usd5y:
   st.header("USD5Y Smart Money tracker")
   df_data = df[df["Asset"] == 'UST 5Y NOTE - CHICAGO BOARD OF TRADE'].iloc[:,1:]
   df_data1 = df_data.copy()
   df_data = fillColor(df_data)
   st.dataframe(df_data)
...
with usd10y:
   st.header("USD10Y Smart Money tracker")
   df_data = df[df["Asset"] == 'UST 10Y NOTE - CHICAGO BOARD OF TRADE'].iloc[:,1:]
   df_data1 = df_data.copy()
   df_data = fillColor(df_data)
   st.dataframe(df_data)
