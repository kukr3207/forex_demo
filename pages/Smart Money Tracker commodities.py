...
commodities,palladium, platinum, silver, copper, gold, cobalt, alumium, steel = st.tabs(["Commodities Overall", 'PALLADIUM', 'PLATINUM', 'SILVER', 'COPPER', 'GOLD', "COBALT", "ALUMIUM", "STEEL"])

      
with commodities:
    l = ['PALLADIUM - NEW YORK MERCANTILE EXCHANGE','PLATINUM - NEW YORK MERCANTILE EXCHANGE','SILVER - COMMODITY EXCHANGE INC.','COPPER- #1 - COMMODITY EXCHANGE INC.','GOLD - COMMODITY EXCHANGE INC.','COBALT - COMMODITY EXCHANGE INC.','ALUMINUM - COMMODITY EXCHANGE INC.','STEEL-HRC - COMMODITY EXCHANGE INC.']

    l1 = ['PALLADIUM', 'PLATINUM', 'SILVER', 'COPPER', 'GOLD',"COBALT", "ALUMIUM", "STEEL",]
...
with gold:
    st.header("Gold Smart Money tracker")
    df_data = df[df["Asset"] == 'GOLD - COMMODITY EXCHANGE INC.'].iloc[:,1:]
...
with palladium:
    st.header("Palladium Smart Money tracker")
    df_data = df[df["Asset"] == 'PALLADIUM - NEW YORK MERCANTILE EXCHANGE'].iloc[:,1:]
    df_data1 = df_data.copy()
    df_data = fillColor(df_data)
    st.dataframe(df_data)
...
with platinum:
    st.header("Platinum Smart Money tracker")
    df_data = df[df["Asset"] == 'PLATINUM - NEW YORK MERCANTILE EXCHANGE'].iloc[:,1:]
    df_data1 = df_data.copy()
    df_data = fillColor(df_data)
    st.dataframe(df_data)
...
with silver:
    st.header("Silver Smart Money tracker")
    df_data = df[df["Asset"] == 'SILVER - COMMODITY EXCHANGE INC.'].iloc[:,1:]
    df_data1 = df_data.copy()
    df_data = fillColor(df_data)
    st.dataframe(df_data)
...
with copper:
    st.header("Copper Smart Money tracker")
    df_data = df[df["Asset"] == 'COPPER- #1 - COMMODITY EXCHANGE INC.'].iloc[:,1:]
    df_data1 = df_data.copy()
    df_data = fillColor(df_data)
    st.dataframe(df_data)
...
with cobalt:
    st.header("Cobalt Smart Money tracker")
    df_data = df[df["Asset"] == 'COBALT - COMMODITY EXCHANGE INC.'].iloc[:,1:]
    df_data1 = df_data.copy()
    df_data = fillColor(df_data)
    st.dataframe(df_data)
...
with alumium:
    st.header("Alumiun Smart Money tracker")
    df_data = df[df["Asset"] == 'ALUMINUM - COMMODITY EXCHANGE INC.'].iloc[:,1:]
    df_data1 = df_data.copy()
    df_data = fillColor(df_data)
    st.dataframe(df_data)
...
with steel:
    st.header("Steel Smart Money tracker")
    df_data = df[df["Asset"] == 'STEEL-HRC - COMMODITY EXCHANGE INC.'].iloc[:,1:]
    df_data1 = df_data.copy()
    df_data = fillColor(df_data)
    st.dataframe(df_data)
